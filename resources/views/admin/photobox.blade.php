...
@section('content')
    <div class="container max-w-4xl mb-8 pt-8">
        ...
        @foreach ($data as $tanggal => $bookings)
            <form action="{{ route('destroyBookingPhotobox', ['date' => $tanggal]) }}" method="POST"
    ...
    <button type="submit" onclick="return confirm('Delete seluruh pesanan pada {{ $tanggal }}?')"><i
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3">Jumlah Bando</th>
                            <th scope="col" class="px-6 py-3">Gambar Bando</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($bookings as $booking)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4">
                                    {{ $booking->jumlah_bando ? $booking->jumlah_bando : 'Tidak ada data terekam' }}</td>
                                <td class="px-6 py-4">
                                    @foreach ($gambar as $item)
                                        @if ($item->photobox_id === $booking->id)
                                            <a href="{{ route('downloadPhoto', ['id' => $item->id]) }}">
                                                <img src="{{ asset('photos/' . $item->nama) }}" alt=""
                                ...
                                <td class="px-6 py-4">
                                    {{ $booking->kendaraan ? $booking->kendaraan : 'Tidak ada data terekam' }}</td>
                                <td class="px-6 py-4">{{ $booking->created_at }}</td>
                                <td class="px-6 py-4">
                                    <form action="{{ route('photobox.addNote', $booking->id) }}" method="POST">
                                        @csrf
                                        <textarea 
                                            name="note" 
                                ...
                                <td class="px-6 py-4">
                                    <form action="{{ route('destroyOneBookingPhotobox', ['id' => $booking->id]) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
        ...
        dark:focus:ring-red-900 mt-10"><i
                    class="fa-solid fa-backward mr-2"></i>Kembali</a>
            {{-- <a href="{{ route('harga') }}" class="inline-block focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300
        font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-green-600 dark:hover:bg-green-700
        dark:focus:ring-red-900 mt-10"><i class="fa-solid fa-tag mr-2"></i>Edit Harga</a> --}}
            <a href="{{ route('indexAdmin') }}"
                class="inline-block focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300
        font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-green-600 dark:hover:bg-green-700
        dark:focus:ring-red-900 mt-10"><i
                    class="fa-solid fa-tag mr-2"></i>Self Photo</a>
            <a href="{{ route('hapusSampah') }}"
        ...
        dark:focus:ring-red-900 mt-10"><i
                    class="fa-solid fa-trash mr-2"></i>Hapus Sampah</a>
