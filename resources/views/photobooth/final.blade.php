...
@section('content')
    ...
    <style>
        /* Tambahkan di file global CSS, misal di resources/css/app.css */
    ...
    <div x-data="{
        ...
        <div x-data="{
            ...
            printTemplate(index) {
                ...
                html2canvas(element, {
                    ...
                    const html = `<html> <head> <title>Print Template</title> <style> @page { size: 4in 6in; /* Ukuran 4R */ margin: 0; } body { margin: 0; display: flex; align-items: center; justify-content: center; background: black; height: 100vh; } img { width: 100%; height: auto; object-fit: contain; } </style> </head> <body> <img src='${image}' alt='Template Print' /> <script>
        ...
        <!-- Tutorial Bar (Floating Bottom) -->
        <div x-show="showTutorialBar" x-transition
            class="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 bg-gray-900/90 text-gray-100 
           border border-gray-700/60 backdrop-blur-md rounded-2xl shadow-2xl 
           px-6 py-3 flex items-center gap-4 max-w-3xl w-[90%] text-sm">
